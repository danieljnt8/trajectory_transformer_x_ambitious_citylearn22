...
class ActionCalculator:
    ...
    def __init__(self, model_num):
        ...
        self.action_space = {}
        self.state_history = {}
        self.observations_data = {agent_id: [] for agent_id in range(5)}
    ...
    def compute_all_actions(self, forecasts, observation_list, return_actions = True):

        observations_data = {agent_id: [] for agent_id in range(len(observation_list))}
        #print("Here")
        ...
        for agent_id, observation in enumerate(observation_list):
            ...
            stage_0_features[agent_id] = features.tolist() + [float(action)]
        ...
        for agent_id, (action, prev_agent_net_energy, next_agent_net_energy, observation) in enumerate(zip(
            ...
            bool_feature = [g_inc, g_dec, g_pos, g_neg, a_inc, a_dec, a_pos, a_neg]
            bool_float = [float(x) for x in bool_feature]

            self.observations_data[agent_id].append(
                stage_0_features[agent_id]+
                bool_float+
                features.tolist()+ [action]
            )
...
class UserAgent:
    ...
    def compute_all_actions(self, observation_list, return_actions= True):
        ...
        #print("Forecasts "+str(np.array(forecasts).shape))
        ...
        if return_actions == True:
            ...
            for agent_ in range(self.num_agents):
                ...
                for calculator in self.calculators:
                    ...
                    #print(calculator.observations_data[agent_])
                    total_observations += np.array(calculator.observations_data[agent_][-1])
                obs_agents.append(total_observations/len(self.calculators))
